	...
	send:function(textMessage, peerUserName) {
        ...
        members:function(message) {
            ...
            $.each(message.data, function() {
                var mTo = $.cometChat.emailToUsername(this);
            	if ($.cometChat.loginUserName == this) { //login user
            		sb[sb.length] = "<span style=\";color: #FF0000;\">" + mTo + "</span><br>";
            	} else { //peer users
            		sb[sb.length] = "<span onclick=\"javascript:createWindow('" + $.cometChat.loginUserName + "', '" + this + "');\"  style=\"cursor: pointer;color: #0000FF;\">" + mTo + "</span><br>";
            	}
            });
